...
void dijkstraCheapest(int source, int destination, int citiesCount, FILE *outputFile)
{
    ...
    fprintf(outputFile, "Cheapest path from %s to %s:\n", cities[source], cities[destination]);
    for (int i = pathIndex - 1; i >= 0; i--)
    {
        fprintf(outputFile, "%s ", cities[path[i]]);
    }
    fprintf(outputFile, "\nCheapest cost: %d\n", dist[destination]);
    fprintf(outputFile, "Maximum weight on this path: %d\n\n", maxWeight);
}
...
void findMaxWeightPathKruskal(int source, int destination, int numCities, FILE *outputFile)
{
    ...
    {
        fprintf(outputFile, "Path with maximum weight capacity from %s to %s:\n", cities[source], cities[destination]);
        for (int i = 0; i < pathIndex; i++)
        {
            fprintf(outputFile, "%s ", cities[path[i]]);
        }
        fprintf(outputFile, "\nMaximum weight capacity: %d\n", maxWeight);
        fprintf(outputFile, "Total price of this path: %d\n\n", totalPrice);
    }
    else
    {
        fprintf(outputFile, "No path found from %s to %s using Kruskal's Algorithm.\n", cities[source], cities[destination]);
    }
}
...
int main()
{
    ...
    dijkstraCheapest(source, destination, MAX_CITIES, cheapestResultsFile);

    // Find the path with the maximum weight capacity and its total price
    findMaxWeightPathKruskal(source, destination, MAX_CITIES, maxWeightResultsFile);

    // Close the files
    fclose(cheapestResultsFile);
    fclose(maxWeightResultsFile);

    printf("Results have been written to files.\n");
    return 0;
}